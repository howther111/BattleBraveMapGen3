...
pix = 200
sky = 100 #最上
highmount = random.randint(0, 100) #以上は高山
forest_line = highmount - random.randint(0, 50) #以上は高山
forest = max(-50, forest_line) #以上は森林
land_line = forest - random.randint(0, 125) #以上は高山
land = max(-100, land_line) #以上は平地
sand_line = land - random.randint(0, 50)
sand = max(-100, sand_line) #以上は砂地
normalsea_line = sand - random.randint(0, 50)
normalsea = max(-100, normalsea_line) #以上は通常水地
deepsea = -100 #以上は深水
shahei_build = random.randint(0, 10) #遮蔽建物の割合
shogai_build = shahei_build - max(0, random.randint(0, shahei_build)) #障害建物の割合
...
def gen(text_x, text_y, csv_flg, text_highmount, text_forest, text_land, text_sand, text_normalsea
    ...
    if not errFlg:
        genmap13.genmap(genxmax, genymax, csv_flg, highmount, forest, land, sand, normalsea, shahei_build, shogai_build)
