...
from generateMapFunc200 import *
...
def gen(text_x, text_y, range_type, color="#FFFFFF"):
    genxmax = xmax
    errFlg = False
    if text_x.isdecimal() or text_x == "":
        genxmax = int(text_x)
    ...
    if not errFlg:
        messagebox.showinfo('範囲タイプ', str(range_type))
        #generateMapFunc200(genxmax, genymax, color)

try:
    root = Tk()
    # ----------- ①Window作成 ----------- #
    root.title('スクウェア範囲生成')  # 画面タイトル設定
    root.geometry('300x250')  # 画面サイズ設定
    ...
    frame7 = Frame(root, width=150, height=100)  # Canvas
    frame8 = Frame(root, width=150, height=100)  # Canvas用ボタン
    ...
    label_7a = Label(frame7, text='タイプ', font=('System', 14))
    label_7a.pack()
    ...
    # ラジオボタン作成
    rdo1 = Radiobutton(frame8, value=0, variable=var, text='範囲')

    rdo2 = Radiobutton(frame8, value=1, variable=var, text='直線')

    rdo3 = Radiobutton(frame8, value=2, variable=var, text='放射')

    rdo1.pack()
    rdo2.pack()
    rdo3.pack()

    button_10a = Button(frame10, text='生成', command=lambda: [gen(entry_2b.get(), entry_5b.get(), var.get(), color='#FFFF00')])
    button_10a.pack(padx=5, pady=10, expand=True)

    button_11a = Button(frame11, text='終了', command=lambda: sys.exit())
