...
def genmap(xsquare=9, ysquare=6, csv_flg=1, highmount=0.5, forest=0.3, land=-0.2,
    ...
    pery = np.zeros((N, N))
    for i in np.random.rand(3):  # パーリンノイズは周波数を変えて何枚か重ねると本領を発揮するらしいのでループして加算
    ...
    outputMinText = ""
    deepwaterBorder = ((squarepixel ** 2) - (int((squarepixel ** 2) / 1.5))) * -3
    waterBorder = ((squarepixel ** 2) - (int((squarepixel ** 2) / 1.5))) * -1
    forestBorder = ((squarepixel ** 2) - (int((squarepixel ** 2) / 1.5))) * 1
    mountBorder = ((squarepixel ** 2) - (int((squarepixel ** 2) / 1.5))) * 3
    for y in range(ysquare):
        for x in range(xsquare):
            ...
            if meanNum < deepwaterBorder:
                outputMapArray[y][x] = -2
                if csv_flg == 1:
                    outputMinText = outputMinText + "深,"
                elif csv_flg == 0:
                    outputMinText = outputMinText + "　,"
            elif meanNum < waterBorder:
                outputMapArray[y][x] = -1
                if csv_flg == 1:
                    outputMinText = outputMinText + "水,"
                elif csv_flg == 0:
                    outputMinText = outputMinText + "　,"
                ...
                if build_seed <= shogai_build:
                    ...
                    for i in range(build_num):
                        ...
                        buildystart = buildy - (buildsize / 2)
                        buildyend = buildy + (buildsize / 2)
                        draw.rectangle((buildxstart, buildystart, buildxend, buildyend), fill=build_col1,
                                       outline=build_col2)
                else:
                    outputMinText = outputMinText + "　,"
