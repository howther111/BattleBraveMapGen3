...
class Program:
    ...
    def __init__(self):
        ...
        try:
            root = Tk()
            canvas = Toplevel(root)  # ← Tk() の代わりに Toplevel を使う
            # ----------- ①Window作成 ----------- #
            ...
            canvasx = len(l[0]) * 50
            canvasy = len(l) * 50
            canvas.title('キャンバス')  # 画面タイトル設定
            canvas.geometry(str(canvasx) + 'x' + str(canvasy))  # 画面サイズ設定
            canvas.canvas_frame = [[0 for j in range(len(l[0]))] for i in range(len(l))]
            canvas.button_frame = [[0 for j in range(len(l[0]))] for i in range(len(l))]
            for y in range(len(l)):
                for x in range(len(l[0])):
                    canvas.canvas_frame[y][x] = Frame(canvas, width=50, height=50)
                    canvas.canvas_frame[y][x].propagate(False)
                    canvas.canvas_frame[y][x].grid(row=y, column=x)
                    # ★ x,y をデフォルト引数で束縛して遅延束縛を回避
                    canvas.button_frame[y][x] = Button(
                        canvas.canvas_frame[y][x],
                        text=l[y][x],
                        font=('System', 28),
